...
def analize(dataframe):
    ...
    dataframe['angle_far'] = talib.LINEARREG_ANGLE(dataframe['close'], timeperiod=500)
    print(dataframe)

#back test using dataframe rolling window
    for win in dataframe.rolling(window = 3):
        if int(win.iloc[[0]]['engulfing']) == 100 and float(win.iloc[[0]]['angle_far']) < 80 and float(win.iloc[[0]]['cci']) > 20:
            if float(win.iloc[[1]]['open']) < float(win.iloc[[2]]['open']):
                call_win = call_win +1
            elif float(win.iloc[[1]]['open']) > float(win.iloc[[2]]['open']):
                call_lose = call_lose + 1
        elif int(win.iloc[[0]]['engulfing']) == -100 and float(win.iloc[[0]]['angle_far']) > -80 and float(win.iloc[[0]]['cci']) < -20 :
            if float(win.iloc[[1]]['open']) > float(win.iloc[[2]]['open']):
                put_win = put_win +1
            elif float(win.iloc[[1]]['open']) < float(win.iloc[[2]]['open']):
                put_lose = put_lose + 1
